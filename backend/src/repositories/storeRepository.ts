...
class StoreRepository {
  async createStore(userId: string, name: string, location: string | null = null): Promise<Store> {
    const text = `
      INSERT INTO stores (user_id, name, location, created_by)
      VALUES ($1, $2, $3, $1) -- created_by is the same as user_id initially
      RETURNING id, user_id, name, location, created_by, created_at, updated_by, updated_at;
    `;
    const rows = await query<Store>(text, [userId, name, location]);
    return rows[0];
  }

  async findStoresByUserId(userId: string): Promise<Store[]> {
    const text = `
      SELECT id, user_id, name, location, created_by, created_at, updated_by, updated_at
      FROM stores
      WHERE user_id = $1;
    `;
